...
Camera::Camera(const proto::SSL_GeometryCameraCalibration &protoCam)
    : position{Eigen::Vector3d(protoCam.derived_camera_world_tx(), protoCam.derived_camera_world_ty(), protoCam.derived_camera_world_tz())},
      ...
      orientation{
          Eigen::Quaterniond(protoCam.q3(), protoCam.q0(), protoCam.q1(), protoCam.q2()).normalized()},  // We normalize here so we don't need to do it in calcuations repeatedly
    ...
    if(protoCam.has_pixel_image_width()){
        imageWidth = protoCam.pixel_image_width();
    }
    if(protoCam.has_pixel_image_height()){
        imageHeight = protoCam.pixel_image_height();
    }
}
...
Eigen::Quaterniond Camera::worldToCamRotation() const { return orientation; }
