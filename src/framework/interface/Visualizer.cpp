...
Visualizer::Visualizer(QWidget* parent)
    : QGraphicsView(parent),
      field{FieldState(DefaultField::DivisionA)} {  // the field constructor above must be the same default constructor as in your ai to make sure they have the same field!
    ...
    fieldRect = QRectF(-(halfLength + margin), -(halfWidth + margin), 2 * (halfLength + margin), 2 * (halfWidth + margin));
...
void Visualizer::updateRobot(const proto::WorldRobot& robot, QMap<uint, Robot>& robots, const proto::RobotInfo& info, const QColor& color) {
...
void Visualizer::drawDetectionRobot(QPainter* painter, const proto::SSL_DetectionRobot& bot, const proto::RobotInfo& info, const QColor& color) {
...
void Visualizer::drawField(QPainter* painter) {
    ...
    for (const LineSegment& line : {field.getLeftLine(), field.getTopLine(), field.getRightLine(), field.getBottomLine(), field.getHalfLine(), field.getCenterLine(),
                                    field.getLeftPenaltyLine(), field.getBottomLeftPenaltyStretch(), field.getTopLeftPenaltyStretch(), field.getRightPenaltyLine(),
                                    field.getBottomRightPenaltyStretch(), field.getTopRightPenaltyStretch()}) {
