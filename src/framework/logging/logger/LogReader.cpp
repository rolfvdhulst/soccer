...
unsigned int LogReader::fileMessageCount() {
...
std::pair<long long int, proto::FrameLog> LogReader::frameAt(unsigned int packet) {
    return readPacket(index[packet]);
}
std::pair<long long int, proto::FrameLog> LogReader::nextFrame() {
    return readPacket(inStream->tellg());
}
void LogReader::resetToStartOfFile() {
    inStream->seekg(startPosition);
}
long long int LogReader::getNextTime() {
    long currentPos = inStream->tellg();
    LogDataHeader dataHeader;
    long long int time = -1;
    if(!inStream->eof() &&  inStream->read((char*) &dataHeader, sizeof(dataHeader)) && !inStream->bad()){
        time = dataHeader.timestamp;
    }
