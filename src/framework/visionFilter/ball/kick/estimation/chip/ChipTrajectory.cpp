...
void ChipState::reflectVelocity(ChipBounceParameters parameters, const Eigen::Vector3d& planeNormal) {
    ...
    Eigen::Vector3d reflection = velocity - 2*(velocity.dot(normalized_normal))*normalized_normal;
    velocity = reflection*parameters.restitution;
}

ChipState ChipState::prediction(Time at_time) const {
    double dt = (at_time-current_time).asSeconds();
...
ChipState::ChipState(Eigen::Vector3d pos, Eigen::Vector3d vel, Time time) :position(std::move(pos)), velocity(std::move(vel)), current_time(time)
{
}
...
std::optional<ChipState> ChipState::predictWithBounces(Time at_time, ChipBounceParameters parameters) const{
    ...
    while( nrBounces < MAXBOUNCES){
        ...
        if(bounce_state){
            if(bounce_state->current_time < at_time){
...
Eigen::Vector3d ChipState::pos() const {
  return position;
}
Time ChipState::time() const {
  return current_time;
}
Eigen::Vector3d ChipState::vel() const {
  return velocity;
}
